...
class ArticleRetrievalAgent:
    ...
    def process_urls(self, urls, search_plan, test_mode=False, max_age_days=None):
        ...
        if test_mode:
            ...
            print("TESTING MODE: Searching for relevant articles")
            print(f"Will retrieve up to {articles_per_substack} articles per Substack")
        else:
            print("Processing all Substacks...")
            articles_per_substack = 200  
            max_substacks = len(urls)
            print(f"Will retrieve up to {articles_per_substack} articles per Substack (with pagination)")
        
        if max_age_days is not None:
            print(f"Filtering for articles newer than {max_age_days} days")
        ...
        for url in urls[:max_substacks]:
            processed_urls += 1
            print(f"Checking Substack {processed_urls}/{max_substacks}: {url}")
            ...
            if not articles and not hasattr(self.substack_client, 'last_had_age_filtering'):
                error_count += 1
                if error_count > 5:
                    print("Multiple consecutive errors detected. Taking a longer break...")
        ...
        print(f"Retrieved {len(all_articles)} articles from {processed_urls} Substacks")
        return all_articles 