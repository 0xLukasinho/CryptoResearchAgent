...
import sys
sys.path.append('..')
from config import OPENAI_API_KEY, MODEL, SUPADATA_MAX_TRANSCRIPTS
...
class YouTubeAgent:
    ...
    def load_youtube_data(self, file_path):
        ...
        try:
            ...
            print(f"Successfully loaded {len(df)} YouTube channels")
            return df
        except Exception as e:
            print(f"Error loading YouTube CSV file: {e}")
        return None
    ...
    def search(self, query, required_terms=None, max_results=5, max_age_days=None, test_mode=False, output_dir=None):
        ...
        try:
            ...
            print(f"Warning: Invalid max_results value '{max_results}', using default of 5")
            max_results = 5
        ...
        for _, channel in channels_df.iterrows():
            ...
            print(f"[YOUTUBE] Processing channel: {channel_name}")
            ...
            try:
                ...
                if test_mode:
                    ...
                    if videos_found_in_test_mode >= 2:
                        print(f"[YOUTUBE] Test mode: Found {videos_found_in_test_mode} matching videos, stopping early")
                        break
                else:
                    all_matching_videos.extend(filtered_videos)
                
            except Exception as e:
                print(f"[YOUTUBE] Error processing channel {channel_name}: {e}")
                continue
        ...
        print(f"[YOUTUBE] Search complete. Returning {len(formatted_results)} relevant videos.")
    ...
    def retrieve_channel_videos(self, channel_id, channel_url, max_age_days):
        ...
        print(f"[YOUTUBE] Retrieved {len(videos)} videos from channel")
        return videos
    ...
    def filter_videos_by_required_terms(self, videos, required_terms):
        ...
        if not required_terms:
            print("[YOUTUBE] No required terms provided - no filtering applied")
            return videos
        ...
        for video in videos:
            ...
            if all_terms_found and any_term_in_title:
                print(f"[YOUTUBE] Video passed filter: {video.get('title')}")
                filtered_videos.append(video)
            else:
                # Increment counter but don't log each filtered video
                filtered_count += 1
        
        print(f"[YOUTUBE] Filtered {len(videos)} videos to {len(filtered_videos)} matching required terms ({filtered_count} filtered out)")
    ...
    def calculate_relevance_scores(self, videos, query):
        ...
        for i, video in enumerate(sorted_videos[:min(3, len(sorted_videos))]):
            print(f"  {i+1}. {video.get('title')} - ({video.get('relevance_score')})")
        print()
    ...
    def fetch_transcripts(self, videos, output_dir, max_transcripts):
        ...
        for video in videos:
            ...
            try:
                ...
                if not video_id:
                    print(f"[YOUTUBE] Error: No video_id for {video.get('title')}")
                    continue
                    
                print(f"[YOUTUBE] Fetching transcript for: {video.get('title')}")
                ...
                if not transcript or not transcript.get('success') or not transcript.get('content'):
                    print(f"[YOUTUBE] Failed to retrieve transcript for: {video.get('title')}")
                    continue
                ...
                if output_dir:
                    ...
                    print(f"[YOUTUBE] Saved transcript to: {transcript_path}")
                
                videos_with_transcripts.append(video)
                
            except Exception as e:
                print(f"[YOUTUBE] Error processing transcript for '{video.get('title')}': {e}")
